...
void exportToFile(const string& filename, const vector<double>& X, const vector<double>& A, const vector<double>& rho,
                  const vector<double>& V, const vector<double>& T, const vector<double>& p, const vector<double>& Ma,
                  const vector<double>& mdot, const vector<double>& U1, const vector<double>& U2,
    ...
    for (size_t i = 0; i < X.size(); ++i) {
        fout << i + 1 << '\t' << X[i] << '\t' << A[i] << '\t' << rho[i] << '\t' << V[i] << '\t' << T[i] << '\t' << p[i]
             << '\t' << Ma[i] << '\t' << mdot[i] << '\t' << U1[i] << '\t' << U2[i] << '\t' << U3[i] << '\n';
    }
}
...
inline double F2(double U1, double U2, double U3) {
    return (U2 * U2) / U1 + (Gamma - 1.0) / Gamma * (U3 - (0.5 * Gamma * U2 * U2) / U1);
}

inline double F3(double U1, double U2, double U3) {
    return (Gamma * U2 * U3 / U1) - (Gamma * (Gamma - 1.0) * U2 * U2 * U2) / (2.0 * U1 * U1);
}
...
int main() {
    ...
    vector<double> X(totalPoints), A(totalPoints), rho(totalPoints), V(totalPoints), T(totalPoints), p(totalPoints),
        Ma(totalPoints), mdot(totalPoints), U1(totalPoints), U2(totalPoints), U3(totalPoints), pU1(totalPoints),
        pU2(totalPoints), pU3(totalPoints), prho(totalPoints), pT(totalPoints), dU1dt(totalPoints), dU2dt(totalPoints),
        dU3dt(totalPoints), pdU1dt(totalPoints), pdU2dt(totalPoints), pdU3dt(totalPoints), dU1dtav(totalPoints),
        dU2dtav(totalPoints), dU3dtav(totalPoints);
    ...
    exportToFile("./output/Q2_initval.csv", X, A, rho, V, T, p, Ma, mdot, U1, U2, U3);
    ...
    for (int iter = 0; iter < timesteps; ++iter) {
        ...
        // 保存第一次迭代结果
        if (iter == 0) {
            exportToFile("./output/Q2_iteration1.csv", X, A, rho, V, T, p, Ma, mdot, U1, U2, U3);
        }
    }

    // 最终结果输出
    exportToFile("./output/Q2_final.csv", X, A, rho, V, T, p, Ma, mdot, U1, U2, U3);
    return 0;
}