...
int main(){
    ...
    p[cellNum+1],Ma[cellNum+1],mdot[cellNum+1],prho[cellNum+1],pT[cellNum+1],pp[cellNum+1];
    ...
    exportToFile("../output/Q3_initval.csv",X,A,rho,V,T,p,Ma,mdot,U1,U2,U3,cellNum);
    ...
    if(i==0){
        ...
        pU1[i] = U1[i] + dU1dt[i] * deltaT + (Cx * abs(p[i+1]-2*p[i]+p[i-1]) * (U1[i+1]-2*U1[i]+U1[i-1])/(p[i+1]+2*p[i]+p[i-1]));
        pU2[i] = U2[i] + dU2dt[i] * deltaT + (Cx * abs(p[i+1]-2*p[i]+p[i-1]) * (U2[i+1]-2*U2[i]+U2[i-1])/(p[i+1]+2*p[i]+p[i-1]));
        pU3[i] = U3[i] + dU3dt[i] * deltaT + (Cx * abs(p[i+1]-2*p[i]+p[i-1]) * (U3[i+1]-2*U3[i]+U3[i-1])/(p[i+1]+2*p[i]+p[i-1]));
    }
    prho[i] = pU1[i]/A[i];
    ...
    //人工粘性
    U1[i] = U1[i] + dU1dtav[i] * deltaT + (Cx * abs(pp[i+1]-2*pp[i]+pp[i-1]) * (pU1[i+1]-2*pU1[i]+pU1[i-1])/(pp[i+1]+2*pp[i]+pp[i-1]));
    U2[i] = U2[i] + dU2dtav[i] * deltaT + (Cx * abs(pp[i+1]-2*pp[i]+pp[i-1]) * (pU2[i+1]-2*pU2[i]+pU2[i-1])/(pp[i+1]+2*pp[i]+pp[i-1]));
    U3[i] = U3[i] + dU3dtav[i] * deltaT + (Cx * abs(pp[i+1]-2*pp[i]+pp[i-1]) * (pU3[i+1]-2*pU3[i]+pU3[i-1])/(pp[i+1]+2*pp[i]+pp[i-1]));
    }
    ...
    if(j==0){
       exportToFile("../output/Q3_iteration1.csv",X,A,rho,V,T,p,Ma,mdot,U1,U2,U3,cellNum);
    }
    }

    //输出timesteps步后的数据
    exportToFile("../output/Q3_final.csv",X,A,rho,V,T,p,Ma,mdot,U1,U2,U3,cellNum);
